...
for n in range(len(DD)):
    ...
    np.save("ER_N{}_M{}_Inter_Access_Num.npy".format(str(N),str(M)),Inter_Access_Num)
    np.save("ER_N{}_M{}_doc_pa.npy".format(str(N),str(M)),doc_pa)
    np.save("ER_N{}_M{}_pa_doc.npy".format(str(N),str(M)),pa_doc)
    Fii = np.array([len(v) for v in doc_pa.values()])
    Fii_hat = Fii-np.sum(Inter_Access_Num,axis=0)
    np.save("ER_N{}_M{}_Fii.npy".format(str(N),str(M)),Fii)
    np.save("ER_N{}_M{}_Fii_hat.npy".format(str(N),str(M)),Fii_hat)
    
    Text_Num = np.empty((T+1,N),dtype=object)
    ...
    Rij = np.zeros((N,N),dtype=object) ## residual capacity from i to j
    ...
    while t < T:
        ...
        leaving = np.array([sum(flux_ij[i]) for i in range(N)]) ### out-degree 
        incoming = np.array([sum(flux_ji[i]) for i in range(N)]) ### in-degree
    ...
    # np.save("ER_N{}_M{}_m{}_Weighted_Adjacency_Matrix.npy".format(str(N),str(M),str(n)),Weighted_Adjacency_Matrix)
    
np.save("ER_N{}_M{}_m10_Patient_Num_ALL.npy".format(str(N),str(M)),Patient_Num_ALL)
np.save("ER_N{}_M{}_m10_Patient_Satisfaction_ALL.npy".format(str(N),str(M)),Patient_Satisfaction_ALL)
np.save("ER_N{}_M{}_m10_Initial_DISS.npy".format(str(N),str(M)),DISS)
np.save("ER_N{}_M{}_m10_Degree_Nodes_ALL.npy".format(str(N),str(M)),Degree_Nodes_ALL)
np.save("ER_N{}_M{}_m10_Degrees_Graph.npy".format(str(N),str(M)),Degrees_Graph)
np.save("ER_N{}_M{}_m10_Quality_Service_ALL.npy".format(str(N),str(M)),Quality_Service_ALL)
np.save("ER_N{}_M{}_m10_In_Degree_ALL.npy".format(str(N),str(M)),In_Degree_ALL)
np.save("ER_N{}_M{}_m10_Out_Degree_ALL.npy".format(str(N),str(M)),Out_Degree_ALL)
